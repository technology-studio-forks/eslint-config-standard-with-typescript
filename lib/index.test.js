...
const getPkgDetails = async () => {
    const readPkgUp = (await (0, inclusion_1.default)('read-pkg-up')).readPackageUp;
...
const extractVersionSpec = (range) => range.split('@').slice(-1)[0];
const equivalents = [...(new eslint_1.Linter()).getRules().keys()]
    .filter(name => Object.prototype.hasOwnProperty.call(eslint_plugin_1.rules, name));
const ourRules = _1.default.rules;
if (ourRules === undefined)
    throw new Error('we seem to be exporting no rules');
const standardRules = eslint_config_standard_1.default.rules;
if (standardRules === undefined)
    throw new Error('`eslint-config-standard` does not seem to be exporting rules');
...
(0, ava_1.default)('Exported rule values do not reference eslint-config-standard ones', (t) => {
    for (const ruleName in eslint_config_standard_1.default.rules) {
        ...
        t.false(ourRules[`@typescript-eslint/${ruleName}`] === eslint_config_standard_1.default.rules[ruleName]);
    }
});
(0, ava_1.default)('npm install args in readme satisfy peerDeps', async (t) => {
    const { pkgJson, pkgPath, ourPeerDeps } = await getPkgDetails();
    const readme = (await (0, promises_1.readFile)((0, path_1.resolve)(pkgPath, '..', 'readme.md'))).toString();
...
(0, ava_1.default)('all direct equivalents are used', (t) => {
...
(0, ava_1.default)('configs of equivalents are supersets of upstream', (t) => {
    equivalents.forEach((ruleName) => {
        ...
        const diff = (0, just_diff_1.diff)({ _: standardRuleConfig }, { _: ourRuleConfig });
        diff.forEach((diff) => {
            if (diff.op !== 'add') {
                t.fail();
                t.log(ruleName, diff);
...
(0, ava_1.default)('all plugin rules are considered', (t) => {
    const intentionallyExcludedRules = [];
    ...
    const assertNotInOurRules = (rule) => {
    ...
    const inexplicablyExcludedRules = Object.keys(eslint_plugin_1.rules)
        .filter((rule) => {
        const typescriptRule = `@typescript-eslint/${rule}`;
...
(0, ava_1.default)('our configuration is compatible with the plugin and parser at bottom of peer dep range', async (t) => {
    const { ourPeerDeps, ourDevDeps } = await getPkgDetails();
    const peerDepRange = ourPeerDeps['@typescript-eslint/eslint-plugin'];
    if (peerDepRange === undefined)
        throw new Error();
    ...
    const minPeerDepVersion = semver_1.default.minVersion(peerDepRange);
    if (minPeerDepVersion === null)
        throw new Error();
    ...
    const config = (0, structured_clone_1.default)(_1.default);
    config.plugins = [typescriptEslintBottomPlugin];
    config.parser = typescriptEslintBottomParser;
    config.rules = Object.fromEntries(Object.entries(ourRules).map(([name, config]) => [
